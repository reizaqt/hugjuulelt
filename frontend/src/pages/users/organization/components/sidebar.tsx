...
// import OrderIcon from "./icons/orderInfoIcon";
...
//         <button onClick={() => navigate("/organization/dashboard")} className={buttonClass}>
...
//             <button
//               onClick={() => navigate("/organization/service")}
//               className={buttonClass}
//             >
...
const Sidebar = ({ onCollapseToggle }: SidebarProps) => {
  ...
  return (
    <div className={`relative ${isCollapsed ? "w-16" : "w-64"} min-h-screen bg-gradient-to-b from-sky-500 to-blue-900 text-white transition-all duration-300 text-sm overflow-hidden`}>
      ...
      <div className="flex flex-col gap-2 mt-4 relative z-10">
        ...
        {!isCollapsed && isOrderOpen && (
          <ul className="ml-6 mt-1 space-y-1">
            <button
              onClick={() => navigate("/organization/appointment/approval")}
              className={buttonClass}
            >
              <ChartBarIcon className="w-5 h-5" />
              Цаг баталгаажуулах
        ...
        >
          ...
          {!isCollapsed && (
            <ChevronLeftIcon
              className={`w-3 h-3 transition-transform ${
                isRegisterOpen ? "-rotate-90" : ""
              }`}
            />
          )}
        </button>

        {!isCollapsed && isRegisterOpen && (
          <ul className="ml-6 mt-1 space-y-1">
            <button
              onClick={() => navigate("/organization/schedule")}
              className={buttonClass}
